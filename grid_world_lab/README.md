...
In this workspace, the GPU environment is already installed one directory above this project. From this folder, use `& '../.venv/Scripts/python.exe'` in place of `python` in the commands below. It contains PyTorch 2.11.0 with CUDA 12.8 and NumPy 2.5.3. The standalone project itself only depends on the packages listed in `requirements.txt`.
...
For each of exactly `data.train_samples` walks:
