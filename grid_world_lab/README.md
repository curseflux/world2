...
The offline `extraction.html` shows the true graph beside a separate model-coordinate map. The second map does not inherit row-major coordinates from the true graph. It fits coordinates jointly from accepted beliefs of the form `source + direction = decoded target`: if the stable constraints say `2 S -> 13` and `13 N -> 2`, node 13 is placed directly below node 2 regardless of their true locations or numeric IDs. Constraint weights combine context count, modal agreement, and mean probe confidence. Disconnected components are translated apart only for display. Contradictory cycles cannot all be drawn exactly, so red dashed edges and the weighted residual report where the model's beliefs fail to form one consistent two-dimensional map.
